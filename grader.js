...
var checkHtmlFile = function(cheerObj, checksfile) {
	...
	var present = cheerObj(checks[ii]).length > 0;
...
var formatOutJson = function(checkJson) {
...
if(require.main == module) {
	...
	.option('-u, --url <url>', 'URL of file to be graded')
	.parse(process.argv);
    if (program.url) {
...
console.log("stuff"); */
	...
	if (result instanceof Error) {
	    console.error('Error: ' + util.format(response.message));
            ...
            formatOutJson(checkJson);
	}
    };
	rest.get(program.url).on('complete', buildfn);
